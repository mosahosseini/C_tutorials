...
def make_graph(
    G,
):  # Här har vi 4 loopar men de innre looparna körs max konstant antal gånger.
    for n1 in G:
        for n2 in G:
            if n1 != n2:
                ...
                for c in range(1, 5):
                    ...
                    word.remove(
                        c1
                    )  # If found we remove the char from word  and its ok because word has constant length.
                if (
                    found
                ):  # Kommer bara vara sann efter loopen ovan om alla 4 c1 gått genom
                    n1.arcs.append(n2)
